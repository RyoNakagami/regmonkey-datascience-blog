...
## Appendix: TCPとUDPの比較表

:::: {.no-border-top-table}

| 特徴   | TCP            | UDP             |
| ---- | -------------- | --------------- |
| OSI参照モデル | トランスポート層 | トランスポート層 |
| 接続形式 | コネクション型        | コネクションレス型       |
| 信頼性  | 高い             | 低い              |
| 順序保証 | あり             | なし              |
| 再送制御 | あり             | なし              |
| 軽量さ  | 遅い（オーバーヘッドあり）  | 速い              |
| 用途例  | SSH, HTTP, FTP | DNS, VoIP, 動画配信, NTP, DHCP |
...
::: {#exm- .custom_problem }
### `/etc/services` から http で始まるサービス名を抽出
<br>
