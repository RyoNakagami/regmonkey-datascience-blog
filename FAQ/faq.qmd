...
[skill の配置場所]{.mini-section}

skill を[どこに置くかで利用できるユーザの範囲]{.regmonkey-bold}が決まる．
...
[名前衝突時の優先順位]{.mini-section}

- 同じ名前の skill が複数レベルに存在する場合，[エンタープライズ > パーソナル > プロジェクト]{.regmonkey-bold} の順で上書きされる
- プラグイン由来の skill は `<plugin-name>:<skill-name>` という[名前空間]{.regmonkey-bold}を持つため，他レベルとは衝突しない
- `.claude/commands/` 配下のコマンドファイルも同じ仕組みで動作するが，[skill と command が同じ名前を持つ場合は skill が優先]{.regmonkey-bold}される
...
- 文章が書けるからといって，誰でも作家になれるわけではないように，コードが書ける → だれでも開発者になれるわけではない（コードモンキー）
...
[レガシーコードとの関係]{.mini-section}

- コードは書いた瞬間から時代遅れになる
- 大事なことは，ソフトウェアの性質と，ソフトウェアは時間の経過とともに変化する必要があることを認識すること


::: {.callout-note}
### なぜWhyが大事なのか

- ある状況で「正しい」アプローチが，別の状況では間違ったアプローチになる可能性は常にある
- 型通りのルールだけでプログラミングという活動を捉えることは難しい
...
- ソフトウェア開発のための軽量なプロセス
- 顧客満足を最優先し，価値あるソフトウェアを速く継続的に提供することを重視
- 

:::

[アジャイルが機能するためには]{.mini-section}

- 共通ゴール，共通理解，共通プラクティス，共通語彙の定義と周知が徹底されていること
- チームが常にフィードバックを受け取り，学ぶこと・改善することができる状態になってること

[アジャイル開発の注意点]{.mini-section}

- スプリント自体が目的ではなく，「小さな塊で仕事をする」ためにスプリントを設ける
...
| 命名規則             | 例                      |
| ---------------- | ---------------------- |
| PascalCase       | `RobotArmController`   |
| camelCase        | `robotArmController`   |
| snake_case       | `robot_arm_controller` |
| kebab-case       | `robot-arm-controller` |
| UPPER_SNAKE_CASE | `ROBOT_ARM_CONTROLLER` |
...
| クラス名 | PascalCase             |
| 型名   | PascalCase             |
| 関数名  | camelCase や snake_case |
| 変数名  | camelCase や snake_case |
| 定数   | UPPER_SNAKE_CASE       |
...
- マネージャーの役割は人を働かせることではなく，人が働けるようにすること
...
- 陳腐な策だが効果があるからこそ多用されるし，多用されるから陳腐になる(ルパート・ケッセルリンク)
